...
<body>
	<h1>${message }</h1>
	<h1>
		用户列表--<a href="/user/add">添加用户</a>---<a href="<%=request.getContextPath() %>/logout">退出登录</a>
	</h1>
	<h2>权限列表</h2>
	...
	<shiro:hasRole name="normal3">normal角色登录显示此内容</shiro:hasRole>

	<shiro:hasAnyRoles name="manager,normal">**manager or admin 角色用户登录显示此内容**</shiro:hasAnyRoles>
	<shiro:principal />
	-显示当前登录用户名
	...
	<ul>
		<c:forEach items="${userList }" var="user">
			<li>用户名：${user.username }----密码：${user.password }----<a
				href="/user/edit/${user.id}">修改用户</a>----<a href="javascript:;"
				class="del" ref="${user.id }">删除用户</a></li>
		</c:forEach>
	</ul>
	<img alt="" src="/static/img/1.jpg">
	<script type="text/javascript"
		src="http://cdn.staticfile.org/jquery/1.9.1/jquery.min.js"></script>
	<script>
		$(function() {
			$(".del").click(function() {
